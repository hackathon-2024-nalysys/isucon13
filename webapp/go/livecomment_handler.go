...
func getLivecommentsHandler(c echo.Context) error {
	...
	livestream := livestreamMap[int64(livestreamID)]
...
func postLivecommentHandler(c echo.Context) error {
	...
	livestreamMap, err := getLivestreams(ctx, tx, []int64{livestreamModel.ID})
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to get livestream: "+err.Error())
	}
	...
	// スパム判定
	var ngwords []*NGWord
	if err := tx.SelectContext(ctx, &ngwords, "SELECT id, user_id, livestream_id, word FROM ng_words WHERE user_id = ? AND livestream_id = ?", livestream.Owner.ID, livestream.ID); err != nil && !errors.Is(err, sql.ErrNoRows) {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to get NG words: "+err.Error())
	}
...
func fillLivecommentResponse(ctx context.Context, tx *sqlx.Tx, livecommentModel LivecommentModel, livestream *Livestream) (Livecomment, error) {
	...
	if livestream == nil {
		livestreamMap, err := getLivestreams(ctx, tx, []int64{livecommentModel.LivestreamID})
		if err != nil {
			return Livecomment{}, err
		}
